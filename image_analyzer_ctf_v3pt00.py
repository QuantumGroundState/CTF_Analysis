...
class CircularFeatureAnalyzer:
    ...
    def _build_tk_controls(self):
        ...
        self.tk_toolbar2 = tk.Frame(container, bg="#181818", padx=8, pady=3)
        ...
        tk.Label(self.tk_side, text="Fit Options", bg="#1f1f1f", fg="#ffffff", font=("Arial", 8, "bold")).pack(anchor="w", pady=(0, 6))
    ...
    def _on_load_image(self, event=None):
        try:
            ...
            self.fig.canvas.manager.set_window_title(
                f"CTF Analyzer  v2.99  \u2014  {os.path.basename(path)}")
    ...
    def _estimate_from_rings(self):
        ...
        n_start_range = (range(-30, 31) if self.reimer_auto
        ...
        for n_start in n_start_range:
            for _rev in _dirs:
                if _rev:
                    # n_start → outermost ring; descend toward inner rings
                    n_arr = np.arange(n_start + n_total - 1, n_start - 1, -1, dtype=float)
                else:
                    # n_start → innermost ring; ascend toward outer rings
