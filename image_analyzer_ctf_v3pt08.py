...
v3.08 (current)
# v3pt05 changes:
#   - Friedel calibration reworked: rubber-band box selection replaces single-click
#   - User draws box around one Friedel mate; local max found inside box
#   - Red dot + 10px circle marks peak; matching circle/dot marks Friedel mate
#   - User inputs known d-spacing (nm); pixel calibration back-calculated from FFT distance
#   - Removed Cal Source (File/Analysis) radio buttons
#   - Added Known d (nm) entry widget to toolbar Band 2
#   - Three mpl events: button_press, motion_notify, button_release on ax_fft

# v3pt06 changes:
#   - Friedel overlay (circles/dots/box) persists after APPLY
#   - User can change d-value and re-apply without redrawing the box
#   - Added File Cal ↔ Friedel Cal toggle button on Band 2 toolbar row
#   - _compute_fft_and_profile gains clear_friedel=True parameter
#   - _on_load_image records _file_px_nm; resets Friedel cal state

# v3pt07 changes:
#   - Calibration math uses peak-to-peak distance ÷ 2 instead of center-to-peak
#   - Line drawn between Friedel mates on FFT display
#   - Manual / Gold (Au) radio buttons for d-value source
#   - Gold combobox auto-fills Known d entry with Au reflection d-spacings

# v3pt08 changes:
#   - Extended Au reflection list from (222) through (444)
#   - Renamed "APPLY AS PIXEL CALIBRATION" button to "APPLY CAL"

v3.03
  # ── Temporal Envelope Fit view (new in this file) ────────────────────────
  # A third "Env Fit" radio button added to the Plot view row in tk_side.
  # When selected, ax_reimer_plot shows detected ring amplitudes (max ▼ red,
  # min ▲ blue) as a scatter vs q, then fits:
  #   E(q) = A · exp(−π²/2 · λ² · Δ² · q⁴)
  # using scipy.optimize.curve_fit (parameters A and Δ, the focal spread in
  # nm).  Horizontal guide lines at E = 1/e (green) and E = 1/4 (orange) are
  # drawn with vertical drop-lines to the x-axis, marking the information
  # limits:
  #   q_ie = (2 / (π² λ² Δ²))^¼          (E = 1/e criterion)
  #   q_14 = (2 ln 4 / (π² λ² Δ²))^¼     (E = 1/4 criterion)
  #   d = 1/q  (nm)
  # Cc is derived as Cc = Δ · E₀/ΔE using the existing dE (eV) and voltage
  # fields, so no new GUI inputs are required.
  # The Reimer text panel (ax_reimer) is overwritten with Δ, Cc, and both
  # information-limit values while the Envelope view is active.
  # New artists (all initially hidden):
  #   env_scat_max, env_scat_min  — ring-amplitude scatter points
  #   env_fit_curve               — fitted envelope curve
  #   env_hline_ie, env_hline_14  — horizontal level lines (1/e, 1/4)
  #   env_vline_ie, env_vline_14  — vertical drop-lines at q_ie, q_14
  #   _env_result_text            — concise Δ/Cc text overlay on the plot
  # New methods:
  #   _hide_envelope_artists()    — sets all env_* artists invisible
  #   _draw_envelope_view()       — fits and renders the envelope plot
  # _apply_reimer_view dispatches to _draw_envelope_view when view=='Envelope'
  # and calls _hide_envelope_artists() when switching to other views.
  # _last_ring_amps (list) caches ring amplitudes alongside _last_ring_q_all
  # so _draw_envelope_view can access them without re-querying the profile.
  # Version strings (window title, corner label, file-load title) updated
  # from v2.97/v2.99 → v3.03 throughout.
  # ── Analysis Angle Slider ────────────────────────────────────────────────
  # A second toolbar row (tk_toolbar2, dark #181818 background) was added
  # directly below the main toolbar and contains all angle / profile-mode
  # controls; it is packed with side="top" between tk_toolbar and tk_side so
  # the canvas area is not affected.
  #
  # var_angle_deg (tk.IntVar, default 0) tracks the current analysis angle in
  # integer degrees and is shared by the slider, arrow-key handler, reset
  # button, and all profile-extraction routines.
  #
  # scale_angle: tk.Scale from −180 to +180 degrees, resolution 1, horizontal
  # orientation, no built-in value readout (showvalue=False).  Bound to
  # _on_angle_change via its command= callback.
  #
  # lbl_angle_val: gold (#ffcc44) tk.Label immediately right of the slider
  # displays the current angle as e.g. "+45°" or "  0°"; updated every time
  # _on_angle_change fires.
  #
  # "0°" reset button calls _on_angle_reset which sets var_angle_deg to 0 and
  # triggers _on_angle_change, snapping the angle back to the x-axis.
  #
  # Left / right arrow keys now call _on_angle_change (±1° per keypress) via
  # the existing mpl key_press_event handler (_on_key_press); the previous
  # DM3-slice stepping behaviour on arrow keys was replaced entirely.
  #
  # ── Profile Mode Radio Buttons ───────────────────────────────────────────
  # var_profile_mode (tk.StringVar, default "Rotational Average") selects
  # between two analysis modes; a pair of tk.Radiobutton widgets placed in
  # tk_toolbar2 to the right of the arrow-key hint label trigger
  # _on_profile_mode_change when switched.
  #
  #   "Rotational Average" — _get_active_profile() returns self.rad_avg_fft,
  #   the full azimuthal average computed by compute_radial_profile_fast();
  #   this is identical to behaviour before v3.03.
  #
  #   "Data Through Slice" — _get_active_profile() returns self._slice_profile,
  #   a 1-D radial profile extracted by bilinear interpolation along the single
  #   line at the current angle (both + and − directions from centre averaged
  #   per radius).
  #
  # ── _compute_slice_profile(angle_deg) ────────────────────────────────────
  # New method.  Uses scipy.ndimage.map_coordinates (order=1, mode='nearest')
  # to sample self.amplitude_spectrum along the line defined by angle_deg.
  # Two passes are made (+1 and −1 sign) and the results summed per integer
  # radius bin then divided by the sample count, giving a noise-averaged
  # single-line profile of the same length as rad_avg_fft.  Returns None if
  # amplitude_spectrum is not yet available or scipy.ndimage is missing.
  #
  # ── _get_active_profile() ────────────────────────────────────────────────
  # New helper method.  Returns self._slice_profile when var_profile_mode ==
  # "Data Through Slice" and _slice_profile is not None; otherwise returns
  # self.rad_avg_fft.  Called by _update_fft_row_markers, _run_fit,
  # _update_peaks_table (vlines), _compute_fft_and_profile (data line), and
  # the fit-context snapshot so that every consumer transparently uses
  # whichever profile is currently active.
  #
  # ── _update_slice_line() ─────────────────────────────────────────────────
  # New method.  Sets fft_slice_line data to a line through (fft_cx, fft_cy)
  # extending ±hypot(w,h) pixels in the angle direction (long enough to cross
  # any FFT image), then sets visible=True; hides the line when mode is
  # "Rotational Average".  Called from _on_angle_change,
  # _on_profile_mode_change, and _compute_fft_and_profile.
  #
  # ── fft_slice_line ───────────────────────────────────────────────────────
  # New Line2D artist added to ax_fft: gold (#ffcc44) dashed line, linewidth
  # 1.0, alpha 0.75, zorder 5, initially invisible.  Added to _fft_overlays
  # so it participates in the blit loop and updates without a full redraw.
  #
  # ── _on_profile_mode_change() ────────────────────────────────────────────
  # New callback.  When switching to "Data Through Slice": computes
  # _slice_profile for the current angle, updates line_rad_fft to show the
  # slice data, calls _update_slice_line() to show the dashed FFT line, then
  # calls _update_fft_row_markers and draw_idle.  When switching back to
  # "Rotational Average": clears _slice_profile, resets line_rad_fft to
  # rad_avg_fft, hides the slice line, and refreshes.
  #
  # ── _on_angle_change() ───────────────────────────────────────────────────
  # New callback (replaces the DM3-only logic in _on_key_press).  Updates
  # lbl_angle_val text, recomputes _slice_profile if in "Data Through Slice"
  # mode, refreshes line_rad_fft, calls _update_slice_line,
  # _update_fft_row_markers, and _blit_fft.
  #
  # ── _on_angle_reset() ────────────────────────────────────────────────────
  # New callback.  Sets var_angle_deg to 0 and calls _on_angle_change.
  #
  # ── _update_fft_row_markers refactored ───────────────────────────────────
  # The sector-average profile (±5° wedge) added in a prior session was
  # removed; the method now reads angle_deg and calls _get_active_profile()
  # to obtain profile_1d — either rad_avg_fft or _slice_profile depending on
  # the radio selection.  Ring detection (top-hat morphology) runs on this
  # profile.  Marker positions on the FFT image are placed at
  # (fft_cx ± r·cos(a), fft_cy ± r·sin(a)) along the chosen angle rather
  # than always on the horizontal row.  _fft_row_max_px and _fft_row_min_px
  # now store ring radii in pixels (not x-column positions) so that
  # px_to_k in _update_peaks_table can compute k = radius / (N·px) directly
  # without subtracting the centre x-coordinate.
  #
  # ── amplitude_spectrum stored as self.amplitude_spectrum ─────────────────
  # _compute_fft_and_profile now stores amplitude_spectrum as an instance
  # variable so _compute_slice_profile and _compute_sector_profile can access
  # it without recomputing the FFT.  Initialised to None in __init__.
  #
  # ── self._slice_profile instance variable ────────────────────────────────
  # Initialised to None in __init__.  Holds the most recently computed
  # single-line profile; invalidated (set to None) when mode switches to
  # "Rotational Average" and recomputed whenever the angle changes in
  # "Data Through Slice" mode or a new FFT is computed.
  #
  # ── Default background model changed to "Exponential" ────────────────────
  # self.bg_model initialised to "Exponential" (was "Lin + Exp"); var_bg_model
  # derives its default from self.bg_model so the combobox also starts on
  # "Exponential" with no separate change needed.
  #
  # ── Default phase constraint changed to "Free" ───────────────────────────
  # var_phase_constrain initialised to "Free" (was "±180°"); the phase
  # parameter is now unconstrained by default, matching the common use-case
  # where the phase origin is unknown; the ±180° radio button still available.
  #
  # ── _run_fit updated for active profile ──────────────────────────────────
  # data_fit and ctf_amp_guess now read from _get_active_profile() instead of
  # self.rad_avg_fft directly; the rad_avg_fft key in the fit-context snapshot
  # dict also stores a copy of the active profile so the callback uses the
  # correct data for the line_rad_fft display, y-axis scaling, and fit-start
  # amplitude guess.
  #
  # ── _update_peaks_table vlines updated for active profile ─────────────────
  # Dashed vertical lines from ring markers to the data curve now read y-values
  # from _get_active_profile() so the line heights match the displayed profile
  # whether in rotational-average or data-through-slice mode.
  #
  # ── _compute_fft_and_profile updated for slice mode ──────────────────────
  # After computing rad_avg_fft, if mode is "Data Through Slice" the method
  # recomputes _slice_profile for the current angle and updates line_rad_fft
  # with the slice data; in "Rotational Average" mode _slice_profile is cleared
  # to None.  _update_slice_line is called before _update_fft_row_markers so
  # the dashed FFT line is correct before marker placement.

v2.97
  - Reimer n/q² view: phase offset φ (from the CTF Phase Offset slider) now
    applies a −φ/q² correction to all scatter y-values so the linear fit
    accounts for the constant phase term; ylabel updates to "(n−φ)/q²" and
    the text table shows the active φ value
  - Reimer Phase fit view: radio button to add a constant phase offset φ to
    the CTF model; slider adjustable from −4π to +4π (resolution 0.01 rad);
    fit subtracts φ before fitting A/B so Δz and Cs estimates remain correct;
    fit curve, CTF markers, and result text all updated to reflect φ
  - Version bumped from v2.97 → v2.98
...
import os
import threading
# PARALLEL: ThreadPoolExecutor used for Reimer n_start grid search and CTF curve_fit.
# numpy/scipy release the GIL during LAPACK calls, so threads give real CPU overlap
# without the serialisation cost of multiprocessing or cross-process pickling.
...
# -- Reimer parallel worker ---------------------------------------------------
# PARALLEL: This function is defined at module level (not inside the class) so
# it is picklable and works with both ThreadPoolExecutor and ProcessPoolExecutor.
# Each call handles one (n_start, _rev) candidate independently — there is no
# shared mutable state between workers.  numpy.polyfit releases the GIL during
# its internal LAPACK dgelsd call, so threads achieve genuine CPU overlap here.
#
# Args tuple: (n_start, _rev, q2_all, phi_reimer, incl, wl, n_total)
#   n_start    – integer ring index assigned to the innermost (or outermost) ring
#   _rev       – bool; True = n_start assigned to outermost ring (descending)
#   q2_all     – 1-D float64 array of q² values for all detected rings
#   phi_reimer – phase offset in radians (0.0 if not in manual mode)
#   incl       – bool array; True = ring is included in the fit
#   wl         – electron wavelength in nm
#   n_total    – total number of rings
#
# Returns: candidate dict on success, None if skipped (too few points / Cs ≤ 0)
def _reimer_candidate_worker(args):
    n_start, _rev, q2_all, phi_reimer, incl, wl, n_total = args

    # Build the n-index array for this candidate
    if _rev:
        # n_start → outermost ring; descend inward
        n_arr = np.arange(n_start + n_total - 1, n_start - 1, -1, dtype=float)
    else:
        # n_start → innermost ring; ascend outward
        n_arr = np.arange(n_start, n_start + n_total, dtype=float)

    y = n_arr / q2_all - phi_reimer / q2_all

    # Restrict fit to included points (need ≥ 2)
    q2_fit = q2_all[incl]
    y_fit  = y[incl]
    if len(q2_fit) < 2:
        return None

    # Linear fit: y = m·q² + b  →  Cs = m/λ³,  Dz = −b/(2λ)
    # PARALLEL: polyfit calls LAPACK and releases the GIL — real parallelism.
    try:
    ...
    Cs_try = m_try / (wl ** 3)       # nm
    dz_try = -b_try / (2.0 * wl)     # nm

    # Reject physically impossible (negative Cs) candidates
    if Cs_try <= 0:
        return None
    ...
    return dict(
        n_start=n_start, reversed=_rev, m=m_try, b=b_try,
...
class CircularFeatureAnalyzer:
    def __init__(self, image_path):
        ...
        self._last_ring_amps   = []      # amplitude at each ring; for Envelope view
        ...
        self._fft_cx_last = 0
        self._fft_cy_last = 0
        # --- Friedel picker state (v3pt08: box-draw mode replaces single-click) ---
        self._friedel_box_start  = None   # (x, y) anchor corner set on mouse-press
        self._friedel_box_end    = None   # (x, y) release corner set on mouse-release
        self._friedel_box_rect   = None   # Rectangle patch (rubber-band box)
        self._friedel_peak_pos   = None   # (px, py) local max found inside drawn box
        self._friedel_mate_pos   = None   # (mx, my) centrosymmetric Friedel partner
        self._friedel_artists    = []     # all overlay patches (circles, dots)
        self._friedel_result     = {}     # last computed q_px, px_size_nm etc.
        # --- Calibration toggle state (v3pt08) ---
        self.px_size_nm         = self.base_cal_nm_per_px  # active pixel size (may be overridden by Friedel cal)
        self._analysis_px_nm    = self.base_cal_nm_per_px  # pixel size used for analysis
        self._file_px_nm        = self.px_size_nm   # original file pixel size; never overwritten after load
        self._friedel_px_nm     = None              # last successfully applied Friedel pixel size
        self._using_friedel_cal = False             # True when Friedel cal is active
        ...
        self._fft_overlays = [
            ...
            self.env_info_circle_ie, self.env_info_circle_14,
    ...
    def _build_ui(self):
        ...
        self.fig.canvas.manager.set_window_title("CTF Analyzer  v3pt08")
        self.fig.text(0.01, 0.99, "v3pt08", color="#00ff88", fontsize=12,
        ...
        # Envelope-fit information-limit rings (shown only in Envelope view)
        self.env_info_circle_ie = Circle((0, 0), 10.0, fill=False, edgecolor="#88ff88", linewidth=1.8, linestyle="-.")
        self.env_info_circle_14 = Circle((0, 0), 10.0, fill=False, edgecolor="#ffaa44", linewidth=1.8, linestyle="-.")
        ...
        self.ax_fft.add_patch(self.env_info_circle_ie)
        self.ax_fft.add_patch(self.env_info_circle_14)
        self.scherzer_circle.set_visible(False)
        self.info_limit_circle.set_visible(False)
        self.env_info_circle_ie.set_visible(False)
        self.env_info_circle_14.set_visible(False)
        ...
        # Dashed line showing the data-through-slice direction on the FFT image
        self.fft_slice_line, = self.ax_fft.plot(
            [], [], color='#ffcc44', linewidth=1.0, linestyle='--',
            alpha=0.75, zorder=5, visible=False
        )
        ...
        # ── Envelope view artists (initially hidden) ──────────────────────────
        self.env_scat_max, = self.ax_reimer_plot.plot(
            [], [], 'v', color='red', markersize=7, zorder=5,
            markeredgewidth=0.6, markeredgecolor='#550000',
            label='_nolegend_', visible=False
        )
        self.env_scat_min, = self.ax_reimer_plot.plot(
            [], [], '^', color='#3399ff', markersize=7, zorder=5,
            markeredgewidth=0.6, markeredgecolor='#002255',
            label='_nolegend_', visible=False
        )
        self.env_fit_curve, = self.ax_reimer_plot.plot(
            [], [], '-', color='#ffcc44', linewidth=2.0, zorder=4,
            label='_nolegend_', visible=False
        )
        # Guide lines: horizontal at E=1/e (green) and E=1/4 (orange)
        self.env_hline_ie, = self.ax_reimer_plot.plot(
            [], [], '--', color='#88ff88', linewidth=1.0, alpha=0.8,
            zorder=3, visible=False
        )
        self.env_hline_14, = self.ax_reimer_plot.plot(
            [], [], '--', color='#ffaa44', linewidth=1.0, alpha=0.8,
            zorder=3, visible=False
        )
        # Vertical drop-lines at q_ie (green) and q_14 (orange)
        self.env_vline_ie, = self.ax_reimer_plot.plot(
            [], [], ':', color='#88ff88', linewidth=1.2, alpha=0.9,
            zorder=3, visible=False
        )
        self.env_vline_14, = self.ax_reimer_plot.plot(
            [], [], ':', color='#ffaa44', linewidth=1.2, alpha=0.9,
            zorder=3, visible=False
        )
        self._env_result_text = self.ax_reimer_plot.text(
            0.97, 0.97, '', transform=self.ax_reimer_plot.transAxes,
            color='#ffcc44', va='top', ha='right', fontsize=8,
            fontfamily='monospace', visible=False, zorder=10
        )
        ...
        # v3pt08: three events drive the rubber-band box selection
        self.fig.canvas.mpl_connect('button_press_event',   self._on_friedel_press)
        self.fig.canvas.mpl_connect('motion_notify_event',  self._on_friedel_drag)
        self.fig.canvas.mpl_connect('button_release_event', self._on_friedel_release)
        ...
        # Overlay artists for per-row triangle coloring in the Reimer table.
        # Positioned identically to reimer_text so line spacing is pixel-perfect.
        self.reimer_tri_red = self.ax_reimer.text(
            0.03, 0.99, "", color='red', va='top', ha='left',
            fontfamily='monospace', fontsize=8, zorder=5)
        self.reimer_tri_blue = self.ax_reimer.text(
            0.03, 0.99, "", color='#3399ff', va='top', ha='left',
            fontfamily='monospace', fontsize=8, zorder=5)
    ...
    def _build_tk_controls(self):
        ...
        # ── Band 1: single row (buttons + dropdowns) ─────────────────────────
        self.tk_toolbar = tk.Frame(container, bg="#202020", padx=8, pady=4)
        self.tk_toolbar.pack(fill="x", side="top")

        # ── Band 2: single row (angle + Friedel) ─────────────────────────────
        self.tk_toolbar2 = tk.Frame(container, bg="#181818", padx=8, pady=4)
        self.tk_toolbar2.pack(fill="x", side="top")

        # ── Friedel state vars ───────────────────────────────────────────────
        self.var_friedel_mode    = tk.BooleanVar(value=False)
        self.var_friedel_d_input = tk.StringVar(value="0.0")
        self.var_friedel_source = tk.StringVar(value="Manual")  # v3pt08: Manual or Gold (Au)
        ...
        self.var_angle_deg = tk.IntVar(value=0)
        self.var_profile_mode = tk.StringVar(value="Rotational Average")
        ...
        self.var_dE = tk.StringVar(value="0.8")
        self.var_alpha = tk.StringVar(value="0.5")
        self.var_phase = tk.StringVar(value="-4.51")
        self.var_phase_constrain = tk.StringVar(value="Free")
        ...
        self.cmb_kv = ttk.Combobox(
            self.tk_toolbar, textvariable=self.var_kv, state="readonly", width=8,
            values=list(ELECTRON_WAVELENGTHS.keys())
        )
        self.cmb_kv.pack(side="left", padx=(4, 8))
        self.cmb_kv.bind("<<ComboboxSelected>>", self._on_kv_change)
        ...
        self.entry_px = ttk.Entry(self.tk_toolbar, textvariable=self.var_px, width=10)
        ...
        tk.Button(
            ...
            padx=3, pady=0, font=("Arial", 8, "bold")
        ...
        # --- Angle + Friedel row (tk_toolbar2) ---
        tk.Label(self.tk_toolbar2, text="Analysis Angle:", bg="#181818", fg="#dddddd").pack(side="left")
        self.scale_angle = tk.Scale(
            self.tk_toolbar2, variable=self.var_angle_deg, from_=-180, to=180,
            orient="horizontal", command=self._on_angle_change,
            length=300, resolution=1, showvalue=False,
            bg="#181818", fg="#dddddd", troughcolor="#3a3a3a", highlightthickness=0
        )
        self.scale_angle.pack(side="left", padx=(4, 4))
        self.lbl_angle_val = tk.Label(self.tk_toolbar2, text="  0°", bg="#181818", fg="#ffcc44",
                                      width=5, anchor="w", font=("Arial", 9, "bold"))
        self.lbl_angle_val.pack(side="left", padx=(0, 8))
        tk.Button(
            self.tk_toolbar2, text="0°", command=self._on_angle_reset,
            bg="#3a3a3a", fg="#ffcc44", activebackground="#555555", relief="flat", padx=6
        ).pack(side="left", padx=(0, 8))
        tk.Label(self.tk_toolbar2, text="(Arrow keys: ±1°)", bg="#181818", fg="#666666",
                 font=("Arial", 8)).pack(side="left")
        tk.Label(self.tk_toolbar2, text="  |", bg="#181818", fg="#444444").pack(side="left", padx=(4, 4))
        for _lbl, _val in (("Rotational Average", "Rotational Average"), ("Data Through Slice", "Data Through Slice")):
            tk.Radiobutton(
                self.tk_toolbar2, text=_lbl, variable=self.var_profile_mode, value=_val,
                command=self._on_profile_mode_change,
                bg="#181818", fg="#dddddd", selectcolor="#2b2b2b",
                activebackground="#181818", activeforeground="#ffffff",
                font=("Arial", 8)
            ).pack(side="left", padx=(0, 6))

        # --- Friedel picker (inline, same row as angle) ---
        self.chk_friedel = tk.Checkbutton(
            self.tk_toolbar2, text="Friedel Picker", variable=self.var_friedel_mode,
            command=self._on_friedel_mode_toggle,
            bg="#181818", fg="#aaddff", selectcolor="#2b2b2b",
            activebackground="#181818", activeforeground="#aaddff",
            font=("Arial", 8, "bold")
        )
        self.chk_friedel.pack(side="left", padx=(0, 8))

        tk.Label(self.tk_toolbar2, text="  |", bg="#181818", fg="#444444").pack(side="left", padx=(4, 4))
        # v3pt08: Manual / Gold radio buttons for d-value source
        for _src in ("Manual", "Gold (Au)"):
            tk.Radiobutton(
                self.tk_toolbar2, text=_src, variable=self.var_friedel_source, value=_src,
                command=self._on_friedel_source_change,
                bg="#181818", fg="#cccccc", selectcolor="#2b2b2b",
                activebackground="#181818", activeforeground="#ffffff",
                font=("Arial", 8)
            ).pack(side="left", padx=(4, 2))
        tk.Label(self.tk_toolbar2, text="  |", bg="#181818", fg="#444444").pack(side="left", padx=(4, 4))
        tk.Label(self.tk_toolbar2, text="Known d (nm):", bg="#181818", fg="#dddddd",
                 font=("Arial", 8)).pack(side="left", padx=(0, 4))
        self.entry_friedel_d = ttk.Entry(self.tk_toolbar2, textvariable=self.var_friedel_d_input,
                                         width=8)
        self.entry_friedel_d.pack(side="left", padx=(0, 8))
        # v3pt08: Gold reflection combobox (hidden until Gold radio selected)
        # v3pt08: extended Au list through (444) — FCC selection rules, a=0.40782 nm
        _gold_values = [
            "Au (111)  0.23540 nm",
            "Au (200)  0.20391 nm",
            "Au (220)  0.14420 nm",
        ...
        self.cmb_gold_reflection = ttk.Combobox(
            self.tk_toolbar2, state="readonly", width=18, values=_gold_values
        )
        self.cmb_gold_reflection.bind("<<ComboboxSelected>>", self._on_gold_reflection_select)
        # hidden by default (Manual mode)
        # self.cmb_gold_reflection.pack(...)  ← not packed yet; shown by _on_friedel_source_change

        self.lbl_friedel_result = tk.Label(
            self.tk_toolbar2, text="Draw a box around one Friedel mate",
            bg="#181818", fg="#888888", font=("Arial", 8), anchor="w"
        )
        self.lbl_friedel_result.pack(side="left", padx=(0, 12))

        self.btn_apply_friedel = tk.Button(
            self.tk_toolbar2, text="APPLY CAL",
            command=self._on_apply_friedel_cal,
            bg="#664400", fg="#ffcc44", activebackground="#886600",
            relief="flat", padx=8, font=("Arial", 8, "bold"),
            state="disabled"
        )
        self.btn_apply_friedel.pack(side="left", padx=(0, 8))

        # v3pt08: toggle between file calibration and Friedel calibration
        self.btn_cal_toggle = tk.Button(
            self.tk_toolbar2, text="Using: File Cal",
            command=self._on_toggle_cal,
            bg="#3a3a3a", fg="#aaaaaa", activebackground="#555555",
            relief="flat", padx=8, font=("Arial", 8, "bold"),
            state="disabled"
        )
        self.btn_cal_toggle.pack(side="left", padx=(8, 0))

        tk.Label(self.tk_side, text="Fit Options", bg="#1f1f1f", fg="#ffffff", font=("Arial", 8, "bold")).pack(anchor="w", pady=(0, 4))
        tk.Label(self.tk_side, text="Background Model", bg="#1f1f1f", fg="#e5e7eb").pack(anchor="w")
        self.cmb_bg_model = ttk.Combobox(
            self.tk_side, textvariable=self.var_bg_model, state="readonly", width=14,
            values=["Linear", "Exponential", "Lin + Exp", "GaussExp"]
        )
        self.cmb_bg_model.pack(anchor="w", pady=(2, 6))
        self.cmb_bg_model.bind("<<ComboboxSelected>>", self._on_bg_model_change)

        # Two-column checkbox grid ─────────────────────────────────────────────
        _chk_grid = tk.Frame(self.tk_side, bg="#1f1f1f")
        _chk_grid.pack(fill="x", pady=(0, 4))
        _col0 = tk.Frame(_chk_grid, bg="#1f1f1f")
        _col0.pack(side="left", anchor="n", fill="x", expand=True)
        _col1 = tk.Frame(_chk_grid, bg="#1f1f1f")
        _col1.pack(side="left", anchor="n", fill="x", expand=True)
        ...
        for txt, var in [
            ...
            _chk(_col0, txt, var).pack(fill="x", pady=1)
        ...
        for txt, var in [
            ...
            _chk(_col1, txt, var).pack(fill="x", pady=1)

        # Bg fix checkboxes — left column (slope/linslope/sigma must share a parent
        # so that pack(after=...) in _on_bg_model_change continues to work)
        self.chk_fix_bg_offset = _chk(_col0, "Fix Bg Offset", self.var_fix_bg_offset)
        self.chk_fix_bg_offset.pack(fill="x", pady=1)

        self.chk_fix_bg_slope = _chk(_col0, "Fix Bg Slope", self.var_fix_bg_slope)
        self.chk_fix_bg_slope.pack(fill="x", pady=1)

        self.chk_fix_bg_linslope = _chk(_col0, "Fix Bg Lin Slope", self.var_fix_bg_linslope)
        self.chk_fix_bg_linslope.pack(fill="x", pady=1)
        self.chk_fix_bg_linslope.pack_forget()  # hidden unless Lin + Exp model

        self.chk_fix_bg_sigma = _chk(_col0, "Fix Bg Sigma", self.var_fix_bg_sigma)
        self.chk_fix_bg_sigma.pack(fill="x", pady=1)
        self.chk_fix_bg_sigma.pack_forget()     # hidden unless GaussExp model

        # Right column: Bg Amp
        self.chk_fix_bg_amp = _chk(_col1, "Fix Bg Amp", self.var_fix_bg_amp)
        self.chk_fix_bg_amp.pack(fill="x", pady=1)

        tk.Label(self.tk_side, text="Initial Guesses", bg="#1f1f1f", fg="#ffffff", font=("Arial", 8, "bold")).pack(anchor="w", pady=(10, 2))
        ...
        _phase_row = tk.Frame(self.tk_side, bg="#1f1f1f")
        _phase_row.pack(fill="x", pady=2)
        tk.Label(_phase_row, text="Phase (rad)", width=10, anchor="w",
                 bg="#1f1f1f", fg="#d1d5db").pack(side="left")
        ttk.Entry(_phase_row, textvariable=self.var_phase, width=7).pack(side="left", padx=(0, 4))
        for _lbl, _val in (("Free", "Free"), ("±180°", "±180°")):
            tk.Radiobutton(
                _phase_row, text=_lbl,
                variable=self.var_phase_constrain, value=_val,
                command=self._on_phase_constrain_change,
                bg="#1f1f1f", fg="#e5e7eb", selectcolor="#2b2b2b",
                activebackground="#1f1f1f", activeforeground="#ffffff",
                font=("Arial", 8)
        ...
        self.var_reimer_inside = tk.StringVar(value="auto")

        reimer_radio_row = tk.Frame(self.tk_side, bg="#1f1f1f")
        reimer_radio_row.pack(fill="x", pady=(10, 2))
        tk.Label(reimer_radio_row, text="Reimer n-Start:", width=14, anchor="w",
                 bg="#1f1f1f", fg="#ffffff", font=("Arial", 8, "bold")).pack(side="left")
        ...
        reimer_inside_row = tk.Frame(self.tk_side, bg="#1f1f1f")
        reimer_inside_row.pack(fill="x", pady=(0, 2))
        tk.Label(reimer_inside_row, text="Largest n:", width=10, anchor="w",
                 bg="#1f1f1f", fg="#d1d5db", font=("Arial", 8)).pack(side="left")
        for _lbl, _val in (("Auto", "auto"), ("+", "+"), ("−", "-")):
            tk.Radiobutton(
                reimer_inside_row, text=_lbl,
                variable=self.var_reimer_inside, value=_val,
        ...
        # "Clear" button on the same row as the spinbox
        tk.Button(
            reimer_n_row, text="Clear",
        ...
        self.var_reimer_curve_src = tk.StringVar(value="reimer")
        reimer_curve_row = tk.Frame(self.tk_side, bg="#1f1f1f")
        reimer_curve_row.pack(fill="x", pady=2)
        tk.Label(reimer_curve_row, text="Green Reimer\nFit Source:", width=10, anchor="w",
                 bg="#1f1f1f", fg="#d1d5db").pack(side="left")
        ...
        # Top-hat detection threshold: fraction of max top-hat value below which
        # candidates are discarded (0.1 %, 0.5 %, 1 %, 5 %, or 10 %).
        self.var_th_thresh = tk.StringVar(value="5%")
        th_thresh_row = tk.Frame(self.tk_side, bg="#1f1f1f")
        th_thresh_row.pack(fill="x", pady=(4, 2))
        tk.Label(th_thresh_row, text="Ring thresh:", width=10, anchor="w",
                 bg="#1f1f1f", fg="#d1d5db").pack(side="left")
        for lbl in ("0.1%", "0.5%", "1%", "5%", "10%"):
            tk.Radiobutton(
                th_thresh_row, text=lbl,
                variable=self.var_th_thresh, value=lbl,
                command=lambda: (
        ...
        tk.Label(reimer_view_row, text="Plot view:", width=10, anchor="w",
                 bg="#1f1f1f", fg="#d1d5db").pack(side="left")
        for lbl, val in (("n/q²", "Reimer"), ("Phase fit", "Phase"), ("Env Fit", "Envelope")):
            tk.Radiobutton(
        ...
        # Phase fit initial guesses — "Phase fit p₀" label left, entries right
        phase_p0_outer = tk.Frame(self.tk_side, bg="#1f1f1f")
        phase_p0_outer.pack(fill="x", pady=(6, 1))
        tk.Label(phase_p0_outer, text="Phase\nfit p₀", bg="#1f1f1f", fg="#aaaaaa",
                 font=("Arial", 8, "italic"), anchor="center", justify="center",
                 width=6).pack(side="left", padx=(0, 4))
        phase_p0_entries = tk.Frame(phase_p0_outer, bg="#1f1f1f")
        phase_p0_entries.pack(side="left", fill="x", expand=True)

        phase_p0_dz_row = tk.Frame(phase_p0_entries, bg="#1f1f1f")
        phase_p0_dz_row.pack(fill="x", pady=1)
        tk.Label(phase_p0_dz_row, text="Δz guess (nm):", width=13, anchor="w",
                 bg="#1f1f1f", fg="#d1d5db").pack(side="left")
        ...
        phase_p0_cs_row = tk.Frame(phase_p0_entries, bg="#1f1f1f")
        phase_p0_cs_row.pack(fill="x", pady=1)
        tk.Label(phase_p0_cs_row, text="Cs guess (mm):", width=13, anchor="w",
                 bg="#1f1f1f", fg="#d1d5db").pack(side="left")
        ...
        # Phase offset toggle (radio buttons)
        self.var_phase_offset_enabled = tk.StringVar(value="none")
        phase_offset_radio_row = tk.Frame(self.tk_side, bg="#1f1f1f")
        phase_offset_radio_row.pack(fill="x", pady=(8, 1))
        tk.Label(phase_offset_radio_row, text="CTF Phase Offset:", width=16, anchor="w",
                 bg="#1f1f1f", fg="#aaaaaa", font=("Arial", 8, "italic")).pack(side="left")
        for _lbl, _val in (("None", "none"), ("Add φ", "manual"), ("Fit φ", "fit")):
            tk.Radiobutton(
                phase_offset_radio_row, text=_lbl,
                variable=self.var_phase_offset_enabled, value=_val,
                command=self._on_phase_offset_toggle,
                bg="#1f1f1f", fg="#e5e7eb", selectcolor="#2b2b2b",
                activebackground="#1f1f1f", activeforeground="#ffffff"
            ).pack(side="left", padx=(0, 4))

        # Phase offset slider (−4π to 4π)
        phase_offset_slider_row = tk.Frame(self.tk_side, bg="#1f1f1f")
        phase_offset_slider_row.pack(fill="x", pady=1)
        self.var_phase_offset = tk.DoubleVar(value=0.0)
        self.phase_offset_val_label = tk.Label(
            phase_offset_slider_row, text="φ = 0.00r (0.0°)",
            bg="#1f1f1f", fg="#d1d5db", width=18, anchor="w"
        )
        self.phase_offset_val_label.pack(side="left")
        self.phase_offset_slider = tk.Scale(
            phase_offset_slider_row,
            from_=-12 * np.pi, to=12 * np.pi,
            orient="horizontal", resolution=0.01, bigincrement=0.1,
            variable=self.var_phase_offset,
            command=self._on_phase_offset_change,
            bg="#1f1f1f", fg="#e5e7eb", troughcolor="#2b2b2b",
            highlightthickness=0, length=120, showvalue=False,
            state="disabled"
        )
        self.phase_offset_slider.pack(side="left", padx=4)
        self.phase_offset_slider.bind("<Left>",  lambda e: self._phase_slider_step(-0.01))
        self.phase_offset_slider.bind("<Right>", lambda e: self._phase_slider_step( 0.01))
        self.phase_offset_slider.bind("<Up>",    lambda e: self._phase_slider_step( 0.01))
        self.phase_offset_slider.bind("<Down>",  lambda e: self._phase_slider_step(-0.01))
    ...
    def _on_phase_offset_toggle(self):
        """Enable/disable the phase-offset slider and re-run the Reimer analysis."""
        mode = self.var_phase_offset_enabled.get()
        self.phase_offset_slider.config(state="normal" if mode == "manual" else "disabled")
        if mode != "manual":
            self.var_phase_offset.set(0.0)
            self.phase_offset_val_label.config(text="φ = 0.00r (0.0°)")
        self._estimate_from_rings()
        self.fig.canvas.draw_idle()

    def _on_phase_offset_change(self, val):
        """Update the phase-offset label (rads + degrees) and re-run the Reimer analysis."""
        phi = float(val)
        deg = np.degrees(phi)
        self.phase_offset_val_label.config(text=f"φ={phi:+.2f}r ({deg:+.1f}°)")
        self._estimate_from_rings()
        self.fig.canvas.draw_idle()

    def _phase_slider_step(self, delta):
        """Nudge the phase-offset slider by delta (radians) via arrow keys."""
        if self.phase_offset_slider.cget("state") == "disabled":
            return
        constrained = (getattr(self, 'var_phase_constrain', None) and
                       self.var_phase_constrain.get() == "±180°")
        lo = -np.pi       if constrained else -12 * np.pi
        hi =  np.pi       if constrained else  12 * np.pi
        new_val = float(np.clip(self.var_phase_offset.get() + delta, lo, hi))
        self.var_phase_offset.set(round(new_val, 3))
        self._on_phase_offset_change(new_val)

    def _on_phase_constrain_change(self):
        """Apply or remove ±180° constraint on the Reimer phase offset slider."""
        constrained = self.var_phase_constrain.get() == "±180°"
        lo = -np.pi if constrained else -12 * np.pi
        hi =  np.pi if constrained else  12 * np.pi
        self.phase_offset_slider.config(from_=lo, to=hi)
        cur = float(self.var_phase_offset.get())
        clamped = float(np.clip(cur, lo, hi))
        if clamped != cur:
            self.var_phase_offset.set(round(clamped, 3))
            self._on_phase_offset_change(clamped)
        self._estimate_from_rings()
        self.fig.canvas.draw_idle()

    def _on_kv_change(self, event=None):
        """Update the default ΔE when the voltage combobox selection changes."""
        _dE_defaults = {"80 kV": "0.8", "300 kV": "1.2"}
        kv = self.var_kv.get()
        if kv in _dE_defaults:
            self.var_dE.set(_dE_defaults[kv])
    ...
    def _make_labeled_entry(self, parent, label, variable):
        ...
        lbl.pack(side="left")
        ent = ttk.Entry(row, textvariable=variable, width=7)
    ...
    def _on_load_image(self, event=None):
        try:
            ...
            _radial_cache.clear()
            self._apply_binning()
            # v3pt08: record original file calibration; reset Friedel cal state
            self._file_px_nm        = self.px_size_nm
            self._friedel_px_nm     = None
            self._using_friedel_cal = False
            if hasattr(self, 'btn_cal_toggle'):
                self._update_cal_toggle_button()
            self._compute_fft_and_profile()
            self.fig.canvas.draw()
            self.fig.canvas.flush_events()
            self.fig.canvas.manager.set_window_title(
                f"CTF Analyzer  v3pt08  \u2014  {os.path.basename(path)}")
    ...
    def _on_key_press(self, event):
        """Left/right arrow keys change the analysis angle by ±1°."""
        if event.key not in ('left', 'right'):
            return
        ang = int(self.var_angle_deg.get())
        ang += 1 if event.key == 'right' else -1
        ang = max(-180, min(180, ang))
        self.var_angle_deg.set(ang)
        self._on_angle_change()

    def _on_angle_change(self, val=None):
        """Called when the angle slider moves or arrow keys change the angle."""
        ang = int(self.var_angle_deg.get())
        self.lbl_angle_val.config(text=f"{ang:+d}°" if ang != 0 else "  0°")
        if self.rad_avg_fft is None:
            return
        # Recompute slice profile if in Data Through Slice mode
        if (hasattr(self, 'var_profile_mode') and
                self.var_profile_mode.get() == 'Data Through Slice'):
            self._slice_profile = self._compute_slice_profile(ang)
            active = self._get_active_profile()
            if active is not None and self.k_axis is not None and len(self.k_axis) == len(active):
                self.line_rad_fft.set_data(self.k_axis, active)
        self._update_slice_line()
        self._update_fft_row_markers(self._fft_cx_last, self._fft_cy_last)
        self._blit_fft()

    def _on_angle_reset(self):
        """Reset analysis angle to 0° (x-axis)."""
        self.var_angle_deg.set(0)
        self._on_angle_change()
    ...
    # ── Friedel pair picker (v3pt08: rubber-band box selection) ──────────────

    def _on_friedel_mode_toggle(self):
        """Enable/disable the Friedel picker; clear artists when turning off."""
        if not self.var_friedel_mode.get():
            self._clear_friedel_artists()
            self.lbl_friedel_result.config(
                text="Draw a box around one Friedel mate", fg="#888888")
            self.btn_apply_friedel.config(state="disabled")
            self.var_friedel_d_input.set("0.0")

    def _on_friedel_press(self, event):
        """Mouse button pressed on ax_fft — record anchor corner of box."""
        if not self.var_friedel_mode.get():
            return
        if event.button != 1:
            return
        if event.inaxes is not self.ax_fft:
            return
        if event.xdata is None or event.ydata is None:
            return
        # Clear previous result before starting a new box
        self._clear_friedel_artists()
        self._friedel_box_start = (event.xdata, event.ydata)
        self._friedel_box_end   = None

    def _on_friedel_drag(self, event):
        """Mouse moved — update the rubber-band rectangle on ax_fft."""
        if not self.var_friedel_mode.get():
            return
        if self._friedel_box_start is None:
            return
        if event.inaxes is not self.ax_fft:
            return
        if event.xdata is None or event.ydata is None:
        ...
        x0, y0 = self._friedel_box_start
        x1, y1 = event.xdata, event.ydata

        # Remove previous rubber-band rectangle
        if self._friedel_box_rect is not None:
            try:
                self._friedel_box_rect.remove()
            except Exception:
                pass
            self._friedel_box_rect = None

        # Draw new rectangle (dashed cyan, no fill)
        ...
        rect = mpatches.Rectangle(
            (rx, ry), rw, rh,
            linewidth=1.2, edgecolor='#00ffff', facecolor='none',
            linestyle='--', zorder=9,
            transform=self.ax_fft.transData
        )
        self.ax_fft.add_patch(rect)
        self._friedel_box_rect = rect
        self.fig.canvas.draw_idle()

    def _on_friedel_release(self, event):
        """Mouse released — finalise box, find local max, draw overlay."""
        if not self.var_friedel_mode.get():
            return
        if event.button != 1:
            return
        ...
        self._friedel_box_end = (event.xdata, event.ydata)

        amp = getattr(self, 'amplitude_spectrum', None)
        if amp is None:
            return

        h, w = amp.shape

        # Normalise box corners to integer pixel bounds
        x0r, y0r = self._friedel_box_start
        x1r, y1r = self._friedel_box_end
        bx0 = max(0, int(min(x0r, x1r)))
        bx1 = min(w, int(max(x0r, x1r)) + 1)
        by0 = max(0, int(min(y0r, y1r)))
        by1 = min(h, int(max(y0r, y1r)) + 1)

        if bx1 <= bx0 or by1 <= by0:
            return   # degenerate box

        # Find local max inside box
        px, py = self._friedel_find_max_in_box(bx0, by0, bx1, by1)
        self._friedel_peak_pos = (px, py)

        # Friedel mate (centrosymmetric partner about FFT centre)
        cx, cy = w / 2.0, h / 2.0
        mx = int(round(2 * cx - px))
        my = int(round(2 * cy - py))
        mx = max(0, min(w - 1, mx))
        my = max(0, min(h - 1, my))
        self._friedel_mate_pos = (mx, my)

        # v3pt08: use peak-to-peak distance ÷ 2 (does not rely on FFT centre accuracy)
        pp_dist = np.hypot(mx - px, my - py)
        q_px    = pp_dist / 2.0
        self._friedel_result = {
            'pp_dist' : pp_dist,
            'q_px'    : q_px,
            'fft_size': min(h, w)
        }

        self._update_friedel_display()

        # Enable APPLY as soon as a valid box exists; d-value is validated at APPLY time
        self.btn_apply_friedel.config(state="normal")

        self.lbl_friedel_result.config(
            text=f"Peak sep = {pp_dist:.1f} px  (÷2 = {q_px:.1f} px)  — enter Known d then APPLY",
            fg="#aaddff"
        )

        # Solution 1: clear anchor so motion_notify no longer draws a new box
        self._friedel_box_start = None

    def _friedel_find_max_in_box(self, bx0, by0, bx1, by1):
        """Return (px, py) of the amplitude_spectrum maximum within the pixel box."""
        amp = self.amplitude_spectrum
        patch = amp[by0:by1, bx0:bx1]
        flat_idx = int(np.argmax(patch))
        dy, dx = np.unravel_index(flat_idx, patch.shape)
        return bx0 + dx, by0 + dy

    def _update_friedel_display(self):
        """Draw box outline, red dots and 10-px circles at peak and Friedel mate."""
        # Remove old circle/dot artists (keep the box rectangle)
        for art in self._friedel_artists:
            try:
                art.remove()
            except Exception:
                pass
        self._friedel_artists.clear()

        if self._friedel_peak_pos is None:
            self.fig.canvas.draw_idle()
            return

        import matplotlib.patches as mpatches

        for (xp, yp), ring_col in (
            (self._friedel_peak_pos, '#aaddff'),
            (self._friedel_mate_pos, '#ffaa44'),
        ):
            # 10-px open circle
            circ = mpatches.Circle(
                (xp, yp), 10, color=ring_col, fill=False, lw=1.5,
                transform=self.ax_fft.transData, zorder=8
            )
            self.ax_fft.add_patch(circ)
            self._friedel_artists.append(circ)

            # Red filled dot (radius 2)
            dot = mpatches.Circle(
                (xp, yp), 2, color='red', fill=True,
                transform=self.ax_fft.transData, zorder=9
            )
            self.ax_fft.add_patch(dot)
            self._friedel_artists.append(dot)

        # v3pt08: draw dashed white line connecting the two Friedel mates
        if self._friedel_peak_pos is not None and self._friedel_mate_pos is not None:
            from matplotlib.lines import Line2D
            px_p, py_p = self._friedel_peak_pos
            px_m, py_m = self._friedel_mate_pos
            line = Line2D(
                [px_p, px_m], [py_p, py_m],
                color='#ffffff', linewidth=0.8, linestyle='--', zorder=7,
                transform=self.ax_fft.transData
            )
            self.ax_fft.add_line(line)
            self._friedel_artists.append(line)

        self.fig.canvas.draw_idle()

    def _clear_friedel_artists(self):
        """Remove all Friedel overlay patches (box + circles + dots) from ax_fft."""
        ...
        self._friedel_box_start  = None
        self._friedel_box_end    = None
        self._friedel_peak_pos   = None
        self._friedel_mate_pos   = None
        self._friedel_result     = {}

    def _on_apply_friedel_cal(self):
        """Back-calculate px_size_nm from the FFT peak distance and user-supplied d-spacing."""
        r = self._friedel_result
        if not r or r.get('q_px', 0) <= 0:
            return

        try:
            d_known = float(self.var_friedel_d_input.get())
        except ValueError:
            self.lbl_friedel_result.config(
                text="Enter a valid d-spacing (nm) before applying", fg="#ff6666")
            return
        if d_known <= 0:
            self.lbl_friedel_result.config(
                text="d-spacing must be greater than zero", fg="#ff6666")
            return

        q_px     = r['q_px']
        fft_size = r['fft_size']

        # From: q_mag = q_px / (fft_size * px_nm)  and  q_mag = 1 / d_known
        # → px_nm = q_px * d_known / fft_size
        new_px_nm = q_px * d_known / fft_size

        self.px_size_nm         = new_px_nm
        self._analysis_px_nm    = new_px_nm
        self._friedel_px_nm     = new_px_nm      # v3pt08: store for toggle
        self._using_friedel_cal = True            # v3pt08: mark Friedel cal as active
        self.var_px.set(f"{new_px_nm:.6f}")

        # v3pt08: save overlay state before _compute_fft_and_profile clears it
        saved_peak   = self._friedel_peak_pos
        saved_mate   = self._friedel_mate_pos
        saved_result = dict(self._friedel_result)
        saved_box_start = self._friedel_box_start
        saved_box_end   = self._friedel_box_end

        # Re-run FFT and fit with the new calibration (skip Friedel clear)
        _radial_cache.clear()
        self._compute_fft_and_profile(clear_friedel=False)
        self._run_fit()
        self.fig.canvas.draw()

        # v3pt08: restore overlay state and redraw
        self._friedel_peak_pos   = saved_peak
        self._friedel_mate_pos   = saved_mate
        self._friedel_result     = saved_result
        self._friedel_box_start  = saved_box_start
        self._friedel_box_end    = saved_box_end
        self._update_friedel_display()

        self.lbl_friedel_result.config(
            text=f"Applied: px = {new_px_nm:.6f} nm/px",
            fg="#88ff88"
        )

        # v3pt08: keep APPLY enabled for re-apply; enable the cal toggle button
        self.btn_apply_friedel.config(state="normal")
        self._update_cal_toggle_button()

    def _on_friedel_source_change(self):
        """Toggle between Manual d-entry and Gold (Au) reflection combobox."""
        if self.var_friedel_source.get() == "Gold (Au)":
            # Show Gold combobox; make d-entry read-only
            self.cmb_gold_reflection.pack(side="left", padx=(0, 8))
            self.entry_friedel_d.config(state="readonly")
        else:
            # Hide Gold combobox; restore d-entry to editable
            self.cmb_gold_reflection.pack_forget()
            self.entry_friedel_d.config(state="normal")

    def _on_gold_reflection_select(self, event=None):
        """Auto-fill the Known d entry from the selected Au reflection."""
        sel = self.cmb_gold_reflection.get()
        if not sel:
            return
        # Extract numeric d-value: second token before ' nm'
        # Format: "Au (111)  0.23540 nm"
        try:
            d_str = sel.split()[-2]   # token before 'nm'
            d_val = float(d_str)
        except (IndexError, ValueError):
            return
        self.var_friedel_d_input.set(f"{d_val:.5f}")
        # Enable APPLY if a box has already been drawn
        if self._friedel_result and self._friedel_result.get('q_px', 0) > 0:
            self.btn_apply_friedel.config(state="normal")

    def _update_cal_toggle_button(self):
        """Refresh the cal toggle button text/colour to reflect the active calibration."""
        if self._friedel_px_nm is None:
            # No Friedel cal applied yet — keep disabled
            self.btn_cal_toggle.config(state="disabled", text="Using: File Cal",
                                       fg="#aaaaaa", bg="#3a3a3a")
        elif self._using_friedel_cal:
            self.btn_cal_toggle.config(state="normal", text="Using: Friedel Cal",
                                       fg="#88ff88", bg="#1a4a1a")
        else:
            self.btn_cal_toggle.config(state="normal", text="Using: File Cal",
                                       fg="#ffcc44", bg="#4a3a00")

    def _on_toggle_cal(self):
        """Toggle between file pixel calibration and Friedel-derived calibration."""
        if self._friedel_px_nm is None:
            return   # nothing to toggle to

        if self._using_friedel_cal:
            # Switch to file calibration
            self.px_size_nm         = self._file_px_nm
            self._analysis_px_nm    = self._file_px_nm
            self._using_friedel_cal = False
        else:
            # Switch to Friedel calibration
            self.px_size_nm         = self._friedel_px_nm
            self._analysis_px_nm    = self._friedel_px_nm
            self._using_friedel_cal = True

        self.var_px.set(f"{self.px_size_nm:.6f}")
        self._update_cal_toggle_button()

        # v3pt08: save/restore Friedel overlay across the recalculation
        saved_peak      = self._friedel_peak_pos
        saved_mate      = self._friedel_mate_pos
        saved_result    = dict(self._friedel_result)
        saved_box_start = self._friedel_box_start
    ...
    # ── End Friedel pair picker (v3pt08) ──────────────────────────────────────
    ...
    def _compute_fft_and_profile(self, clear_friedel=True):
        # Clear Friedel overlay only when loading a new image (not on recalibration)
        if clear_friedel:
            self._clear_friedel_artists()
        ...
        # Recompute slice profile with the new FFT if in Data Through Slice mode
        if (hasattr(self, 'var_profile_mode') and
                self.var_profile_mode.get() == 'Data Through Slice'):
            ang = int(self.var_angle_deg.get()) if hasattr(self, 'var_angle_deg') else 0
            self._slice_profile = self._compute_slice_profile(ang)
        else:
            self._slice_profile = None
        ...
        if eff_px_now > 0:
            ...
            if hasattr(self, 'line_rad_fft'):
                active = self._get_active_profile()
                display_profile = active if active is not None else self.rad_avg_fft
                self.line_rad_fft.set_data(k_now, display_profile)
    ...
    def _compute_slice_profile(self, angle_deg):
        """Sample the amplitude spectrum along a single line at angle_deg from x-axis.

        Both +angle and +angle+180° directions are sampled and the results averaged
        per radius.  Bilinear interpolation via scipy.ndimage.map_coordinates.
        Returns an ndarray (length max_radius+1) or None.
        """
        if self.amplitude_spectrum is None:
            return None
        try:
            from scipy.ndimage import map_coordinates as _mc
        except ImportError:
            return None
        h, w = self.amplitude_spectrum.shape
        cx, cy = w // 2, h // 2
        max_radius = int(np.hypot(w / 2.0, h / 2.0))
        ...
        spec_f64 = self.amplitude_spectrum.astype(np.float64)
        for sign in (+1.0, -1.0):
            xs = cx + sign * radii * cos_a
            ys = cy + sign * radii * sin_a
            valid = (xs >= 0) & (xs < w) & (ys >= 0) & (ys < h)
            ri   = radii[valid].astype(int)
            vals = _mc(spec_f64, np.array([ys[valid], xs[valid]]),
    ...
    def _get_active_profile(self):
        """Return the 1D radial profile to use for ring detection and CTF fitting.

        'Data Through Slice' mode returns the single-line interpolated profile;
        'Rotational Average' returns the full azimuthal average.
        """
        if (hasattr(self, 'var_profile_mode') and
                self.var_profile_mode.get() == 'Data Through Slice' and
                self._slice_profile is not None):
            return self._slice_profile
        return self.rad_avg_fft

    def _update_slice_line(self):
        """Update the dashed FFT overlay line to show the current slice direction."""
        if not hasattr(self, 'fft_slice_line') or self.fft_img_log is None:
            return
        in_slice_mode = (hasattr(self, 'var_profile_mode') and
                         self.var_profile_mode.get() == 'Data Through Slice')
        if not in_slice_mode:
            self.fft_slice_line.set_visible(False)
            return
        h, w = self.fft_img_log.shape
        cx, cy = w // 2, h // 2
        angle_rad = np.radians(int(self.var_angle_deg.get()) if hasattr(self, 'var_angle_deg') else 0)
        cos_a = float(np.cos(angle_rad))
        sin_a = float(np.sin(angle_rad))
        # Extend line to the image diagonal so it always crosses the full FFT
        R = float(np.hypot(w, h))
        self.fft_slice_line.set_data(
            [cx - R * cos_a, cx + R * cos_a],
            [cy - R * sin_a, cy + R * sin_a]
        )
        self.fft_slice_line.set_visible(True)

    def _on_profile_mode_change(self):
        """Called when the Rotational Average / Data Through Slice radio changes."""
        if self.rad_avg_fft is None:
            return
        angle_deg = int(self.var_angle_deg.get()) if hasattr(self, 'var_angle_deg') else 0
        if self.var_profile_mode.get() == 'Data Through Slice':
            self._slice_profile = self._compute_slice_profile(angle_deg)
        else:
            self._slice_profile = None
        # Refresh CTF data line
        active = self._get_active_profile()
        if active is not None and self.k_axis is not None and len(self.k_axis) == len(active):
            self.line_rad_fft.set_data(self.k_axis, active)
        self._update_slice_line()
    ...
    def _compute_sector_profile(self, angle_deg, half_width_deg=5.0):
        """Radial profile averaged within a ±half_width_deg angular sector.

        Both the +angle and +angle+180° directions are included so the full
        line through the FFT centre contributes.  Returns an ndarray of length
        max_radius+1 (same indexing as rad_avg_fft) or None if no spectrum.
        """
        if self.amplitude_spectrum is None:
            return None
        h, w = self.amplitude_spectrum.shape
        cx, cy = w // 2, h // 2
        y_arr, x_arr = np.ogrid[0:h, 0:w]
        dx = (x_arr - cx).astype(np.float32)
        dy = (y_arr - cy).astype(np.float32)
        r = np.sqrt(dx * dx + dy * dy)
        angle_rad = np.radians(angle_deg)
        cos_a = float(np.cos(angle_rad))
        sin_a = float(np.sin(angle_rad))
        # Perpendicular (cross) distance from the line; within sector when small
        cross = np.abs(-dx * sin_a + dy * cos_a)
        r_safe = np.maximum(r, 1e-6)
        within = (cross / r_safe) < np.sin(np.radians(half_width_deg))
        within[cy, cx] = True
        max_radius = int(np.hypot(w / 2.0, h / 2.0))
        r_int = np.round(r).astype(np.int32)
        mask = within & (r_int <= max_radius)
        r_m = r_int[mask]
        v_m = self.amplitude_spectrum[mask]
        n = max_radius + 1
        profile = np.bincount(r_m, weights=v_m.astype(np.float64), minlength=n)
        counts = np.bincount(r_m, minlength=n)
        valid = counts > 0
        profile[valid] /= counts[valid]
        return profile[:n].astype(np.float64)

    def _update_fft_row_markers(self, fft_cx, fft_cy):
        """Find max/min rings within the current fit ROI along the chosen analysis
        angle and place red/blue dot markers on the FFT image at that angle.

        A ±5° sector profile at the current analysis angle is used instead of
        the full rotational average so the detection reflects the chosen direction.
        """
        ...
        angle_deg = int(self.var_angle_deg.get()) if hasattr(self, 'var_angle_deg') else 0
        active = self._get_active_profile()
        profile_1d = active if active is not None else self.rad_avg_fft

        # --- ROI in radial pixels (matching the blue/green circles) ---
        r_lo = int(self.fit_start_radius)
        r_hi = int(self.fit_end_radius)
        max_r = len(profile_1d) - 1
        r_lo = max(1, min(r_lo, max_r - 2))
        r_hi = max(r_lo + 2, min(r_hi, max_r))

        # Slice the sector profile to the ROI only
        roi_raw = profile_1d[r_lo : r_hi + 1].astype(np.float64)
        roi_len = len(roi_raw)
        ...
        if _HAS_MORPHOLOGY and roi_len > 10:
            ...
            # Threshold: ignore peaks below the chosen fraction of the top-hat max
            _th_map  = {"0.1%": 0.001, "0.5%": 0.005, "1%": 0.01, "5%": 0.05, "10%": 0.10}
            _th_frac = _th_map.get(getattr(self, 'var_th_thresh', None) and
                                   self.var_th_thresh.get(), 0.05)
            wth_thresh = float(np.max(wth)) * _th_frac if wth.max() > 0 else 0.0
            bth_thresh = float(np.max(bth)) * _th_frac if bth.max() > 0 else 0.0
        ...
        # Place markers along the current analysis angle direction (both ± sides)
        cos_a = float(np.cos(np.radians(angle_deg)))
        sin_a = float(np.sin(np.radians(angle_deg)))
        max_xs, max_ys, min_xs, min_ys = [], [], [], []
        for r in th_max_radii:
            for sign in (+1, -1):
                px = fft_cx + sign * r * cos_a
                py = fft_cy + sign * r * sin_a
        ...
        # Persist ring radii (in pixels) for _update_peaks_table k-value conversion
        self._fft_row_max_px = list(th_max_radii)
        self._fft_row_min_px = list(th_min_radii)
        self._fft_cx_last    = fft_cx
        self._fft_cy_last    = fft_cy

        if max_xs:
            self.fft_row_max.set_data(np.array(max_xs), np.array(max_ys))
        else:
            self.fft_row_max.set_data([], [])

        if min_xs:
            self.fft_row_min.set_data(np.array(min_xs), np.array(min_ys))
        else:
    ...
    def _update_peaks_table(self):
        ...
        def px_to_k(radii):
            # _fft_row_max/min_px now stores ring radii in pixels, not column positions
            if not radii:
                return np.array([])
            return np.sort(np.array(radii, dtype=float) / (N * eff_px))
        ...
        if hasattr(self, '_th_vlines'):
            ...
            _vline_profile = self._get_active_profile()
            if self.k_axis is not None and _vline_profile is not None:
                for k_val in self._th_max_ks:
                    idx = int(np.argmin(np.abs(self.k_axis - k_val)))
                    y_val = float(_vline_profile[idx])
                    ln, = self.ax_ctf.plot(
                        [k_val, k_val], [0, y_val],
        ...
        # ── Reimer fit first so cached m/b are available for the RA table ───
        self._estimate_from_rings()

        # ── RA Rings table (top-hat detections only) ──────────────────────
        _rq_all   = getattr(self, '_last_ring_q_all', None)
        _rn_arr   = getattr(self, '_last_ring_n_arr', None)
        _has_n    = (_rq_all is not None and _rn_arr is not None
                     and len(_rq_all) == len(_rn_arr)
                     and len(_rq_all) > 0)

        def _lookup_n(q):
            """Return the Reimer n index for the ring closest to q."""
            idx = int(np.argmin(np.abs(_rq_all - q)))
            return int(_rn_arr[idx])

        def _ra_section(label, qs):
            rows = [label]
            if _has_n:
                rows.append("  # q(1/nm)  d(nm)   n")
                rows.append("  " + "─" * 22)
            else:
                rows.append("  # q(1/nm)  d(nm)")
                rows.append("  " + "─" * 18)
            if qs:
                for i, q in enumerate(qs):
                    if q > 1e-9:
                        if _has_n:
                            rows.append(
                                f"  {i+1:<2} {q:6.4f}  {1.0/q:6.3f}  {_lookup_n(q):>3}"
                            )
                        else:
                            rows.append(f"  {i+1:<2} {q:6.4f}  {1.0/q:6.3f}")
            else:
                rows.append("  (none)")
            return rows
    ...
    def _estimate_from_rings(self):
        ...
        def _clear_plot():
            ...
            if hasattr(self, 'reimer_tri_red'):
                self.reimer_tri_red.set_text("")
                self.reimer_tri_blue.set_text("")
        ...
        # Phase offset for Reimer correction (−φ/q² term)
        _phi_en  = getattr(self, 'var_phase_offset_enabled', None)
        _phi_var = getattr(self, 'var_phase_offset', None)
        phi_reimer = (float(_phi_var.get())
                      if (_phi_en and _phi_en.get() == "manual" and _phi_var) else 0.0)
        ...
        # ── Amplitude at each detected ring ───────────────────────────────────
        # Look up the active-profile value at each ring's k-position so the
        # table can show how much signal each extremum contributes.
        _amp_profile = self._get_active_profile()
        _ring_amps = []
        if (_amp_profile is not None and
                hasattr(self, 'k_axis') and self.k_axis is not None and
                len(self.k_axis) > 0):
            for _qi in q_all:
                _idx = int(np.argmin(np.abs(self.k_axis - _qi)))
                _ring_amps.append(float(_amp_profile[_idx]))
        else:
            _ring_amps = [float('nan')] * n_total
        ...
        # Cache geometry for phase-χ / envelope view (n_arr cached per-branch below)
        self._last_ring_q_all  = q_all.copy()
        self._last_ring_types  = types[:]
        self._last_ring_incl   = incl.copy()
        self._last_ring_wl     = wl
        self._last_ring_amps   = _ring_amps[:]

        # ── Search over n_start values ────────────────────────────────────────
        n_start_range = (range(-50, 51) if self.reimer_auto
                         else [int(self.reimer_n_start_manual)])

        # In "−" mode n_start is assigned to the OUTERMOST (largest-q) ring and
        # n_arr descends inward, so the highest-q ring carries the most-negative n.
        # "auto" + reimer_auto tests BOTH directions for every n_start and picks
        # the globally best R² so the indexing direction is data-driven.
        _n_mode_pre = (self.var_reimer_inside.get()
                       if getattr(self, 'var_reimer_inside', None)
                       else "r2")
        _reversed_n = (_n_mode_pre == "-")

        # Directions to test per n_start value
        if _n_mode_pre == "auto" and self.reimer_auto:
            _dirs = [False, True]   # test ascending then descending
        else:
            _dirs = [_reversed_n]

        # PARALLEL: Build a flat list of all (n_start, _rev) argument tuples.
        # Previously this was a sequential nested for-loop; now every candidate
        # is dispatched to a thread pool simultaneously.  Because numpy.polyfit
        # releases the GIL during its LAPACK call, threads achieve real CPU
        # overlap — typically 4–30× faster on a multi-core machine (depends on
        # core count and the size of n_start_range, which can be 61 × 1-or-2 =
        # up to 122 independent tasks).  Worker logic lives in the module-level
        # _reimer_candidate_worker() so it is picklable if ever switched to
        # ProcessPoolExecutor.  max_workers is capped at logical CPU count to
        # avoid spawning far more threads than the OS can schedule efficiently.
        _worker_args = [
            (n_start, _rev, q2_all, phi_reimer, incl, wl, n_total)
            for n_start in n_start_range
            for _rev in _dirs
        ]
        _n_workers = min(len(_worker_args), (os.cpu_count() or 4))
        with ThreadPoolExecutor(max_workers=_n_workers,
                                thread_name_prefix="reimer") as _pool:
            _raw_results = list(_pool.map(_reimer_candidate_worker, _worker_args))

        # Filter out None returns (skipped candidates: too few points or Cs ≤ 0)
        valid_candidates = [c for c in _raw_results if c is not None]
        ...
        if valid_candidates:
            _n_mode = _n_mode_pre
            if _n_mode == "auto":
                # Both directions were tested — pick the globally best R²;
                # the winning candidate's 'reversed' flag records which won.
                best = max(valid_candidates, key=lambda c: c['r2'])
            elif _n_mode == "+":
                best = max(valid_candidates,
                           key=lambda c: (c['n_start'] + n_total - 1)
                           if (c['n_start'] + n_total - 1) > 0 else -1)
            elif _n_mode == "-":
                best = max(valid_candidates,
                           key=lambda c: abs(c['n_start']) if c['n_start'] < 0 else -1)
            else:
                best = max(valid_candidates, key=lambda c: c['r2'])

        # Resolve actual direction from winning candidate
        if best is not None:
            _reversed_n = best.get('reversed', _reversed_n)
        ...
        if best is None:
            ...
            y_fb       = n_arr_fb / q2_all - phi_reimer / q2_all
            ...
            if hasattr(self, 'reimer_scat_max'):
                ...
                max_y = [yi for yi, ti in zip(y_fb, types) if ti == 'max']
                min_x = [qi**2 for qi, ti in zip(q_all, types) if ti == 'min']
                min_y = [yi for yi, ti in zip(y_fb, types) if ti == 'min']
                self.reimer_scat_max.set_data(max_x, max_y)
                self.reimer_scat_min.set_data(min_x, min_y)
                # Draw fit-parameter line only in Auto mode with "Fit" source
                if (self.reimer_auto and
                        getattr(self, 'var_reimer_curve_src', None) and
            ...
            lines = [
                ...
                f"  n₀=1 (display only)",
                "",
                f"  {'n':>3}     {'q(1/nm)':>7} {'n/q²':>7} {'amp':>9}",
                "  " + "─" * 37,
            ]
            _red_lines  = [""] * len(lines)
            _blue_lines = [""] * len(lines)
            for i, (ni, qi, ti, yi, ai) in enumerate(zip(n_arr_fb, q_all, types, y_fb, _ring_amps)):
                excl_tag = " *" if i in self._reimer_excluded else ""
                _amp_str = f"{ai:>9.4g}" if np.isfinite(ai) else f"{'—':>9}"
                lines.append(
                    f"  {int(ni):>3}      {qi:>7.4f} {yi:>7.4f} {_amp_str}{excl_tag}"
                )
                if ti == 'max':
                    _red_lines.append("       \u25bc")
                    _blue_lines.append("")
        ...
        # Always reflect the chosen n_start in the spinbox
        if hasattr(self, 'var_reimer_n_start'):
            self.var_reimer_n_start.set(n_start)
        m, b    = best['m'], best['b']
        self._reimer_m = m          # cache for RA rings table fit column
        self._reimer_b = b
        ...
        y       = best['y']
        incl    = best['incl']
        # Enforce: largest |n| always corresponds to largest q, with the
        # sequence remaining contiguous.  The worker always produces a
        # contiguous integer range, so only a direction check is needed —
        # if the first element has a larger absolute value than the last,
        # flip the whole array so the high-magnitude end lands at the
        # largest q.  This avoids the oscillation that arises when sorting
        # by abs() on a range that spans zero.
        if abs(n_arr[0]) > abs(n_arr[-1]):
            n_arr = n_arr[::-1].copy()
        y = n_arr / q2_all - phi_reimer / q2_all
        ...
        # Only overwrite Initial Guesses when guess mode is "Reimer Auto".
        # In "Manual" mode the user owns those fields; Reimer must not clobber them.
        _push_ok = not (getattr(self, 'var_guess_mode', None) and
                        self.var_guess_mode.get() == "Manual")
        if _push_ok and hasattr(self, 'var_defocus') and hasattr(self, 'var_cs'):
            self.var_defocus.set(f"{dz_nm:.1f}")
            self.var_cs.set(f"{Cs_mm:.4f}")
            self.current_cs = Cs_mm   # keep internal state in sync

        n_used = int(incl.sum())
        _ntag = ("rev−" if _reversed_n else
                 ("auto" if self.reimer_auto else "manual"))
        mode_tag = f"({_ntag},{src_label})"
        ...
        phi_row = (f"  {'φ (rad)':<13} {phi_reimer:>+8.3f}\n" if phi_reimer != 0.0 else "")
        lines = [
            "── Reimer Eq. 6.50 ──────",
            f"  n₀={n_start} {mode_tag}",
            *([ f"  φ = {phi_reimer:+.3f} rad  (−φ/q² applied)" ] if phi_reimer != 0.0 else []),
            "",
            f"  {'Cs (mm)':<13} {format(Cs_mm, f'>8.{_cs_dec}f')} {_cs_err_str}",
            f"  {'Defocus (nm)':<13} {format(dz_nm, f'>8.{_dz_dec}f')} {_dz_err_str}",
            f"  {'R²':<13} {r2:>8.5f}",
            f"  {'χ²_red':<13} {chi2r:>8.4f}   σ_y={sigma_y:.4f}",
            f"  {'Used/Total':<13} {n_used:>4d}/{n_total:<4d}",
            f"  {'λ (nm)':<13} {wl:>8.5f}",
            "",
            f"  {'n':>3}     {'q(1/nm)':>7} {'resid':>7} {'amp':>9}",
            "  " + "─" * 37,
        ]
        _red_lines  = [""] * len(lines)
        _blue_lines = [""] * len(lines)
        for i, (ni, qi, ti, yi, ypi, ai) in enumerate(
                zip(n_arr, q_all, types, y, y_pred, _ring_amps)):
            excl_tag = " *" if i in self._reimer_excluded else ""
            _amp_str = f"{ai:>9.4g}" if np.isfinite(ai) else f"{'—':>9}"
            lines.append(
                f"  {int(ni):>3}      {qi:>7.4f} {yi-ypi:>+7.4f} {_amp_str}{excl_tag}"
            )
            if ti == 'max':
                _red_lines.append("       \u25bc")
                _blue_lines.append("")
        ...
        lines += _footer
        _red_lines  += [""] * len(_footer)
        _blue_lines += [""] * len(_footer)
        self.reimer_text.set_text("\n".join(lines))
        ...
        max_x = [qi ** 2 for qi, ti in zip(q_all, types) if ti == 'max']
        max_y = [yi for yi, ti in zip(y, types) if ti == 'max']
        min_x = [qi ** 2 for qi, ti in zip(q_all, types) if ti == 'min']
        min_y = [yi for yi, ti in zip(y, types) if ti == 'min']
        ...
        if len(q2_all) >= 2:
            ...
            x_hi   = q2_all.max() * 1.15
            x_line = np.linspace(x_lo, x_hi, 150)
            _use_fit_src = (self.reimer_auto and
                            getattr(self, 'var_reimer_curve_src', None) and
            ...
            if _use_fit_src:
                ...
                # Manual mode always uses the Reimer polyfit directly
                m_line, b_line = m, b
        ...
        for ann in self._reimer_annots:
            ann.remove()
        self._reimer_annots.clear()
        for i, (ni, qi, yi) in enumerate(zip(n_arr, q_all, y)):
    ...
    def _hide_envelope_artists(self):
        """Make all temporal-envelope view artists invisible (plot + FFT circles)."""
        for a in (self.env_scat_max, self.env_scat_min, self.env_fit_curve,
                  self.env_hline_ie, self.env_hline_14,
                  self.env_vline_ie, self.env_vline_14,
                  self._env_result_text):
            a.set_visible(False)
        self.env_info_circle_ie.set_visible(False)
        self.env_info_circle_14.set_visible(False)
        self._blit_fft()
        # Restore the Reimer axes legend when leaving Envelope view
        _leg = self.ax_reimer_plot.get_legend()
        if _leg is not None:
    ...
    def _hide_reimer_artists(self):
        """Make all Reimer n/q² artists invisible."""
        for a in (self.reimer_scat_max, self.reimer_scat_min,
                  self.reimer_fit_line, self._reimer_incl_circles):
            a.set_visible(False)
        for ann in self._reimer_annots:
    ...
    def _apply_reimer_view(self, q_all, n_arr, types, incl, wl):
        ...
        view_val = view.get() if view else 'Reimer'

        if view_val == 'Phase':
            self._hide_reimer_artists()
            self._hide_envelope_artists()
            self._draw_phase_view(q_all, n_arr, types, incl, wl)

        elif view_val == 'Envelope':
            self._hide_reimer_artists()
            self._hide_phase_artists()
            self._draw_envelope_view(q_all, types, incl, wl)
            ...
            _phi_en2  = getattr(self, 'var_phase_offset_enabled', None)
            _phi_var2 = getattr(self, 'var_phase_offset', None)
            _phi2 = (float(_phi_var2.get())
                     if (_phi_en2 and _phi_en2.get() == "manual" and _phi_var2) else 0.0)
            ylabel = "(n − φ) / q²  (nm²)" if _phi2 != 0.0 else "n / q²  (nm²)"
            self.ax_reimer_plot.set_title("Reimer  n/q²  vs  q²",
                                          color="#44dd99", pad=4, fontsize=12)
            self.ax_reimer_plot.set_xlabel("q²  (nm⁻²)", color="#aaa",
                                           fontsize=10, labelpad=2)
            self.ax_reimer_plot.set_ylabel(ylabel, color="#aaa",
                                           fontsize=10, labelpad=2)

    def _draw_envelope_view(self, q_all, types, incl, wl):
        """Fit the temporal coherence envelope via linearisation on ax_reimer_plot.

        Linearisation
        ─────────────
          E(q) = A · exp(−π²λ²Δ²q⁴/2)
          ⟹  ln(E) = ln(A) + c₁·q⁴   where  c₁ = −π²λ²Δ²/2

        This is a straight line in (q⁴, ln E) space, so we fit with
        np.polyfit(q⁴, ln(amplitude), 1).

        The slope gives Δ directly:
          c₁ < 0  →  Δ = √(−2c₁ / (π²λ²))

        Information limits (q where signal falls to fraction f of A):
          E = f·A  →  c₁·q⁴ = ln f
          E = 1/e  →  q_ie = (2 / (π²λ²Δ²))^¼   [in ln-space: y = ln(A)−1]
          E = 1/4  →  q_14 = (2ln4/(π²λ²Δ²))^¼  [in ln-space: y = ln(A)−ln4]

        Plot axes: x = q⁴ (nm⁻⁴),  y = ln(amplitude)
        Scatter: ln(max) fitted (red ▼) + ln(min) reference (blue ▲)
        Fit line: extends from q⁴=0 to q⁴_max
        ...
        ax.set_title("Envelope Fit: ln(amp) vs q\u2074  (line fit to maxima)", color="#44dd99", pad=4, fontsize=10)
        ax.set_xlabel("q\u2074  (nm\u207b\u2074)", color="#aaa", fontsize=10, labelpad=2)
        ax.set_ylabel("ln(Amplitude)", color="#aaa", fontsize=10, labelpad=2)

        # Hide the Reimer legend — it lists Reimer artists that are not shown here
        _leg = ax.get_legend()
        if _leg is not None:
            _leg.set_visible(False)

        # ── Gather cached amplitudes ───────────────────────────────────────────
        amps_raw = getattr(self, '_last_ring_amps', None)
        if (amps_raw is None or len(amps_raw) == 0 or
                len(amps_raw) != len(q_all)):
            self._hide_envelope_artists()
            self._env_result_text.set_text("No amplitude\ndata available.")
            self._env_result_text.set_visible(True)
            return

        amps  = np.array(amps_raw, dtype=float)
        q_arr = np.array(q_all,    dtype=float)

        try:
            dE_eV = float(self.var_dE.get())
        except (ValueError, AttributeError):
            dE_eV = 0.8
        kv_ev = getattr(self, 'current_kv_ev', 80000.0)

        # ── Convert to (q⁴, ln(amp)) space; filter non-positive amplitudes ────
        # Only include points not excluded via the n/q² toggle (incl mask)
        max_idx = np.array([i for i, t in enumerate(types) if t == 'max' and incl[i]], dtype=int)
        min_idx = np.array([i for i, t in enumerate(types) if t == 'min' and incl[i]], dtype=int)
        ...
        def _q4_lna(idx):
            ...
            q_sel = q_arr[idx]
            a_sel = amps[idx]
            ok = np.isfinite(a_sel) & (a_sel > 0)
            return q_sel[ok] ** 4, np.log(a_sel[ok])

        q4_max, lna_max = _q4_lna(max_idx)
        q4_min, lna_min = _q4_lna(min_idx)

        # ── Scatter in ln space ───────────────────────────────────────────────
        self.env_scat_max.set_data(q4_max, lna_max)
        self.env_scat_min.set_data(q4_min, lna_min)
        self.env_scat_max.set_visible(True)
        self.env_scat_min.set_visible(True)

        # ── Linear fit to ln(max) vs q⁴: ln(f) = c0 + c1·q⁴ ─────────────────
        result_lines = [
            "\u2500\u2500 Temporal Envelope \u2500\u2500\u2500\u2500\u2500\u2500",
            "  ln(E)=ln(A)+c\u2081\u00b7q\u2074",
            "  c\u2081=\u2212\u03c0\u00b2\u03bb\u00b2\u0394\u00b2/2  (slope)",
        ...
        if len(q4_max) >= 2:
            try:
                coeffs  = np.polyfit(q4_max, lna_max, 1)
                c1, c0  = float(coeffs[0]), float(coeffs[1])

                if c1 >= 0:
                    result_lines += ["  Slope \u22650: not physical."]
                else:
                    delta_fit = float(np.sqrt(-2.0 * c1 / (np.pi ** 2 * wl ** 2)))
                    A_fit     = float(np.exp(c0))

                    # R² in ln-space
                    lna_pred = c1 * q4_max + c0
                    ss_res   = float(np.sum((lna_max - lna_pred) ** 2))
                    ss_tot   = float(np.sum((lna_max - float(np.mean(lna_max))) ** 2))
                    r2       = 1.0 - ss_res / ss_tot if ss_tot > 1e-30 else 0.0

                    lam2  = wl ** 2
                    denom = np.pi ** 2 * lam2 * delta_fit ** 2
                    # ln-space intercepts for each criterion:
                    #   E=1/e  → ln(A/e)  = c0 - 1;      q⁴_ie = -1 / c1
                    #   E=1/4  → ln(A/4)  = c0 - ln4;    q⁴_14 = -ln4 / c1
                    lny_ie  = c0 - 1.0
                    lny_14  = c0 - np.log(4.0)
                    q4_ie   = -1.0        / c1   # q⁴ where line = c0-1
                    q4_14   = -np.log(4.0) / c1  # q⁴ where line = c0-ln4
                    q_ie    = q4_ie  ** 0.25
                    q_14    = q4_14  ** 0.25
                    d_ie    = 1.0 / q_ie
                    d_14    = 1.0 / q_14

                    Cc_mm = ((delta_fit * kv_ev / dE_eV) / 1.0e6
                             if dE_eV > 0 and kv_ev > 0 else float('nan'))
                    fit_ok = True

                    # ── Feed Cc back into the entry field when Auto mode is on ──
                    if (np.isfinite(Cc_mm) and
                            getattr(self, 'reimer_auto', False) and
                            hasattr(self, 'var_cc')):
                        self.var_cc.set("{:.4f}".format(Cc_mm))

            except Exception as _ex:
                result_lines += ["  Fit failed:", f"  {str(_ex)[:45]}"]
        else:
            result_lines += ["  Need \u22652 maxima."]

        if fit_ok:
            q4_max_val = float(np.max(q_arr)) ** 4

            # ── Fitted line ───────────────────────────────────────────────────
            x_line = np.linspace(0.0, q4_max_val * 1.15, 400)
            self.env_fit_curve.set_data(x_line, c1 * x_line + c0)
            self.env_fit_curve.set_visible(True)

            # ── Horizontal guide lines at ln-criterion levels ──────────────────
            # green: E=1/e  (y = c0 - 1 = ln(A/e))
            # orange: E=1/4 (y = c0 - ln4 = ln(A/4))
            self.env_hline_ie.set_data([0.0, q4_ie],  [lny_ie, lny_ie])
            self.env_hline_ie.set_visible(True)
            self.env_hline_14.set_data([0.0, q4_14], [lny_14, lny_14])
            self.env_hline_14.set_visible(True)

            # ── Vertical drop-lines at q⁴_ie and q⁴_14 ───────────────────────
            # Drop from the fit line down to the bottom of the visible range
            all_lna = np.concatenate([lna_max, lna_min]) if len(lna_min) else lna_max
            y_bot = float(np.min(all_lna)) - 0.5
            self.env_vline_ie.set_data( [q4_ie,  q4_ie],  [y_bot, lny_ie])
            self.env_vline_ie.set_visible(True)
            self.env_vline_14.set_data([q4_14, q4_14], [y_bot, lny_14])
            self.env_vline_14.set_visible(True)

            # ── Axis limits ───────────────────────────────────────────────────
            ax.set_xlim(0.0, q4_max_val * 1.2)
            y_top = c0 + abs(c0 - y_bot) * 0.08
            ax.set_ylim(y_bot, y_top)

            # ── Overlay text on the plot (top-left) ───────────────────────────
            cc_str = "{:.3f} mm".format(Cc_mm) if np.isfinite(Cc_mm) else "\u2014"
            self._env_result_text.set_text(
                # Linearisation and fit
                "ln(E)=ln(A)+c\u2081\u00b7q\u2074,  c\u2081=\u2212\u03c0\u00b2\u03bb\u00b2\u0394\u00b2/2\n"
                "  \u0394=(\u22122c\u2081/\u03c0\u00b2\u03bb\u00b2)\u00bd\n"
                "  c\u2081={:.4g} nm\u2074  ln(A)={:.3f}\n".format(c1, c0) +
                "  R\u00b2={:.5f}\n".format(r2) +
                "  \u0394={:.3f} nm  Cc\u2248{}\n".format(delta_fit, cc_str) +
                "  (\u0394E={:.2f} eV)\n".format(dE_eV) +
                "\n"
                # E=1/e criterion
                "E=1/e \u2192 q\u2074=\u22121/c\u2081 = (2/\u03c0\u00b2\u03bb\u00b2\u0394\u00b2)\n"
                ...
                "\n"
                # E=1/4 criterion
                "E=1/4 \u2192 q\u2074=\u2212ln4/c\u2081 = (2ln4/\u03c0\u00b2\u03bb\u00b2\u0394\u00b2)\n"
                "  ln level: ln(A)\u2212ln4 = {:.3f}\n".format(lny_14) +
                "  q={:.4f} nm\u207b\u00b9   d={:.4f} nm".format(q_14, d_14)
            )
            self._env_result_text.set_visible(True)

            # ── Info limit rings on the FFT ───────────────────────────────────
            N  = getattr(self, 'N_dimension', None)
            px = getattr(self, 'base_cal_nm_per_px', None)
            bf = getattr(self, 'bin_factor', 1)
            cx = self._fft_cx_last
            cy = self._fft_cy_last
            if N and px:
                self.env_info_circle_ie.set_center((cx, cy))
                self.env_info_circle_ie.set_radius(q_ie * N * px * bf)
                self.env_info_circle_ie.set_visible(True)
                self.env_info_circle_14.set_center((cx, cy))
                self.env_info_circle_14.set_radius(q_14 * N * px * bf)
                self.env_info_circle_14.set_visible(True)
            self._blit_fft()

            # ── Full results in Reimer text panel ─────────────────────────────
            cc_row = ("  Cc (mm)        {:>8.4f}".format(Cc_mm)
                      if np.isfinite(Cc_mm) else "  Cc: \u2014 (need dE>0)")
            result_lines += [
                "",
                "  c\u2081  = {:>+12.4e} nm\u2074".format(c1),
                "  ln(A) = {:>8.4f}".format(c0),
                "  R\u00b2    = {:>8.5f}".format(r2),
                "",
                "  \u0394 (nm)         {:>8.3f}".format(delta_fit),
                cc_row,
                "  \u0394E={:.2f} eV  E\u2080={:.0f} kV".format(dE_eV, kv_ev / 1000),
                "",
                # E=1/e with formula
                "  \u2500\u2500 E=1/e \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500",
                "  q=(2/\u03c0\u00b2\u03bb\u00b2\u0394\u00b2)\u00bc",
                "  ln level = ln(A)\u22121 = {:.4f}".format(lny_ie),
                "  q (nm\u207b\u00b9)     {:>8.4f}".format(q_ie),
                "  d (nm)         {:>8.4f}".format(d_ie),
                "",
                # E=1/4 with formula
                "  \u2500\u2500 E=1/4 \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500",
                "  q=(2ln4/\u03c0\u00b2\u03bb\u00b2\u0394\u00b2)\u00bc",
                "  ln level = ln(A)\u2212ln4 = {:.4f}".format(lny_14),
                "  q (nm\u207b\u00b9)     {:>8.4f}".format(q_14),
                "  d (nm)         {:>8.4f}".format(d_14),
            ]
        else:
            for a in (self.env_fit_curve, self.env_hline_ie, self.env_hline_14,
                      self.env_vline_ie, self.env_vline_14):
                a.set_visible(False)
            self.env_info_circle_ie.set_visible(False)
            self.env_info_circle_14.set_visible(False)
            self._blit_fft()
            self._env_result_text.set_visible(False)

        # ── Update the Reimer text panel ──────────────────────────────────────
        if hasattr(self, 'reimer_text'):
            self.reimer_text.set_text("\n".join(result_lines))
            self.reimer_text.set_color('#44dd99' if fit_ok else '#888888')
        if hasattr(self, 'reimer_tri_red'):
            self.reimer_tri_red.set_text("")
            self.reimer_tri_blue.set_text("")
        self.fig.canvas.draw_idle()
    ...
    def _draw_phase_view(self, q_all, n_arr, types, incl, wl):
        ...
        # Phase offset (from radio + slider; 0 if disabled)
        _phi_enabled = getattr(self, 'var_phase_offset_enabled', None)
        _phi_var     = getattr(self, 'var_phase_offset', None)
        _phi_mode    = _phi_enabled.get() if _phi_enabled else "none"
        phi = float(_phi_var.get()) if (_phi_var and _phi_mode == "manual") else 0.0
        ...
        try:
            ...
            if len(q_fit) >= 2:
                ...
                if _phi_mode == "fit":
                    def _chi_model_phi(q, A, B, phi_f):
                        return A * q ** 2 + B * q ** 4 + phi_f
                    _phi_constrained = (getattr(self, 'var_phase_constrain', None) and
                                        self.var_phase_constrain.get() == "±180°")
                    _phi_lo = -np.pi if _phi_constrained else -12 * np.pi
                    _phi_hi =  np.pi if _phi_constrained else  12 * np.pi
                    popt, pcov = _curve_fit(_chi_model_phi, q_fit, chi_fit, p0=[A0, B0, 0.0],
                                           bounds=([-np.inf, -np.inf, _phi_lo],
                                                   [ np.inf,  np.inf, _phi_hi]))
                    A, B, phi = float(popt[0]), float(popt[1]), float(popt[2])
                    var = np.clip(np.diag(pcov), 0.0, None)
                    perr = np.sqrt(var)
                    if _phi_var:
                        _phi_var.set(round(phi, 3))
                    self.phase_offset_val_label.config(text=f"φ = {phi:+.2f} rad")
                else:
                    popt, pcov = _curve_fit(_chi_model, q_fit, chi_fit - phi, p0=[A0, B0])
                ...
                chi_curve = _chi_model(q_curve, A, B) + phi
                self.phase_fit_curve.set_data(q_curve, chi_curve)
                self.phase_fit_curve.set_visible(True)

                # Solve A·u + B·u² = χ_target − φ for u = q² (underfocus: A<0, B>0)
                if A < 0 and B > 0:
                    def _q_for_chi(chi_t):
                        # Account for phase offset: curve = A*q²+B*q⁴+phi, so solve A*u+B*u² = chi_t - phi
                        chi_eff = chi_t - phi
                        disc = A ** 2 + 4.0 * B * chi_eff
                        if disc < 0:
                            return None
                ...
                phi_str = f"  {'φ (rad)':<10} {phi:>+9.3f}\n" if phi != 0.0 else ""
                result_str = (
                    f"  χ = A·q² + B·q⁴{'  +  φ' if phi != 0.0 else ''}\n"
                    f"  CTF = sin(χ)\n"
        ...
        if len(all_q) >= 2:
            ...
            # Always include y=0; extend further for −π level if underfocused
            y_lo = min(all_chi.min() - ypad, -ypad * 0.5)
            y_hi = max(all_chi.max() + ypad, ypad * 0.5)
            if _ctf_annot:
    ...
    def _run_fit(self, event=None):
        ...
        _active_profile = self._get_active_profile()
        if _active_profile is None:
            _active_profile = self.rad_avg_fft

        k_fit = self.k_axis[start_idx:end_idx]
        data_fit = _active_profile[start_idx:end_idx]

        ctfamp_text = self.var_ctfamp.get().strip().lower()
        if ctfamp_text == 'auto':
            ctf_amp_guess = _active_profile[start_idx]
        else:
            try: ctf_amp_guess = float(self.var_ctfamp.get())
            except ValueError: ctf_amp_guess = _active_profile[start_idx]
        ...
        if not self.fix_phase:
            _phase_constrained = (getattr(self, 'var_phase_constrain', None) and
                                  self.var_phase_constrain.get() == "±180°")
            p0.append(phase_guess)
            lower_bounds.append(-np.pi if _phase_constrained else -np.inf)
            upper_bounds.append( np.pi if _phase_constrained else  np.inf)
        ...
        ctx = dict(
            ...
            k_axis=self.k_axis.copy(), rad_avg_fft=(_active_profile.copy() if _active_profile is not None else self.rad_avg_fft.copy()),
        ...
        # PARALLEL: Immediately repaint "Fitting..." before the thread starts so
        # the user sees feedback right away rather than a frozen interface.
        self.fig.canvas.draw_idle()

        mf = ctx['model_func']

        # PARALLEL: Guard flag prevents a second click from launching an
        # overlapping fit while one is already running.  Without this, two
        # threads could race to write into the same ctx dict and corrupt results.
        if getattr(self, '_fit_running', False):
            return
        self._fit_running = True

        # PARALLEL: _fit_worker runs scipy curve_fit in a background daemon
        # thread.  scipy's Levenberg-Marquardt solver calls LAPACK/BLAS routines
        # that release the GIL, so the thread achieves real CPU parallelism —
        # the Tkinter main loop (GUI redraws, slider interaction, etc.) stays
        # fully responsive during the optimisation.
        #
        # Why threading rather than multiprocessing here:
        #   - curve_fit is a single iterative task, not a batch — no fan-out
        #     benefit from multiple processes.
        #   - BLAS already releases the GIL, giving true parallelism.
        #   - threading avoids pickling the large ctx dict and model closure.
        #
        # Thread safety note: _fit_worker only writes into ctx (a local dict
        # snapshot) — it never touches self.* directly.  All self.* mutations
        # happen inside _apply_fit_results(), which is scheduled back onto the
        # Tkinter main thread via after(0, ...) so there is no race condition.
        def _fit_worker():
            try:
                popt, pcov = curve_fit(
                ...
                ctx['residuals_fit'] = (ctx['data_fit'] -
                                        ctx['fit_curve_full'][ctx['start_idx']:ctx['end_idx']])
                ...
                try:
                    ctx['fit_curve_full'] = mf(ctx['k_axis'], *ctx['p0'])
                except Exception:
                    ctx['fit_curve_full'] = np.full_like(ctx['k_axis'], np.nan)
            finally:
                # PARALLEL: Clear the running flag then hand results back to the
                # main thread.  after(0, ...) is the correct Tkinter mechanism for
                # posting a callback from a non-GUI thread — it is thread-safe and
                # runs as soon as the event loop is next idle.
                self._fit_running = False
                self.fig.canvas.get_tk_widget().after(
                    0, lambda: self._apply_fit_results(ctx)
                )

        # PARALLEL: Daemon=True means the thread will not block Python from
        # exiting if the user closes the window while a fit is still running.
        _fit_thread = threading.Thread(
            target=_fit_worker,
            daemon=True,
            name="ctf_curve_fit",   # visible in debuggers / profilers
        )
        _fit_thread.start()
