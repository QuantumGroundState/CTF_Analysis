...
class CircularFeatureAnalyzer:
    def __init__(self, image_path):
        ...
        self._fft_cx_last = 0
        self._fft_cy_last = 0
        # --- Friedel picker state (v3pt06: box-draw mode replaces single-click) ---
        ...
        # --- Calibration toggle state (v3pt06) ---
    ...
    def _build_ui(self):
        ...
        self.fig.canvas.manager.set_window_title("CTF Analyzer  v3pt06")
        self.fig.text(0.01, 0.99, "v3pt06", color="#00ff88", fontsize=12,
        ...
        # v3pt06: three events drive the rubber-band box selection
    ...
    def _build_tk_controls(self):
        ...
        self.btn_apply_friedel = tk.Button(
            self.tk_toolbar2, text="APPLY AS PIXEL CALIBRATION",
        ...
        # v3pt06: toggle between file calibration and Friedel calibration
    ...
    def _on_load_image(self, event=None):
        try:
            ...
            _radial_cache.clear()
            self._apply_binning()
            # v3pt06: record original file calibration; reset Friedel cal state
            ...
            self.fig.canvas.manager.set_window_title(
                f"CTF Analyzer  v3pt06  \u2014  {os.path.basename(path)}")
    ...
    # ── Friedel pair picker (v3pt06: rubber-band box selection) ──────────────
    ...
    def _on_friedel_release(self, event):
        ...
        # Store pixel distance from FFT centre for APPLY
        q_px = np.hypot(px - cx, py - cy)
        self._friedel_result = {'q_px': q_px, 'fft_size': min(h, w)}
        ...
        self.lbl_friedel_result.config(
            text=f"FFT dist = {q_px:.1f} px  — enter Known d then APPLY",
            fg="#aaddff"
        )
    ...
    def _on_apply_friedel_cal(self):
        ...
        self._friedel_px_nm     = new_px_nm      # v3pt06: store for toggle
        self._using_friedel_cal = True            # v3pt06: mark Friedel cal as active
        self.var_px.set(f"{new_px_nm:.6f}")

        # v3pt06: save overlay state before _compute_fft_and_profile clears it
        ...
        # v3pt06: restore overlay state and redraw
        ...
        # v3pt06: keep APPLY enabled for re-apply; enable the cal toggle button
        self.btn_apply_friedel.config(state="normal")
        self._update_cal_toggle_button()
    ...
    def _on_toggle_cal(self):
        ...
        # v3pt06: save/restore Friedel overlay across the recalculation
        saved_peak      = self._friedel_peak_pos
        saved_mate      = self._friedel_mate_pos
        saved_result    = dict(self._friedel_result)
        saved_box_start = self._friedel_box_start
    ...
    # ── End Friedel pair picker (v3pt06) ──────────────────────────────────────
