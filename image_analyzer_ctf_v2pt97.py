...
class CircularFeatureAnalyzer:
    ...
    def _build_ui(self):
        ...
        self.fig.canvas.manager.set_window_title("CTF Analyzer  v2.99")
        self.fig.text(0.01, 0.99, "v2.99", color="#00ff88", fontsize=12,
    ...
    def _build_tk_controls(self):
        ...
        tk.Label(self.tk_side, text="CTF Phase Offset", bg="#1f1f1f", fg="#aaaaaa",
                 font=("Arial", 9, "italic")).pack(anchor="w", pady=(8, 1))
        self.var_phase_offset_enabled = tk.BooleanVar(value=False)
        phase_offset_radio_row = tk.Frame(self.tk_side, bg="#1f1f1f")
        phase_offset_radio_row.pack(fill="x", pady=1)
        tk.Radiobutton(
            phase_offset_radio_row, text="None",
            variable=self.var_phase_offset_enabled, value=False,
            command=self._on_phase_offset_toggle,
            bg="#1f1f1f", fg="#e5e7eb", selectcolor="#2b2b2b",
            activebackground="#1f1f1f", activeforeground="#ffffff"
        ).pack(side="left", padx=(0, 6))
        tk.Radiobutton(
            phase_offset_radio_row, text="Add φ",
            variable=self.var_phase_offset_enabled, value=True,
            command=self._on_phase_offset_toggle,
            bg="#1f1f1f", fg="#e5e7eb", selectcolor="#2b2b2b",
            activebackground="#1f1f1f", activeforeground="#ffffff"
        ).pack(side="left")
        ...
        self.phase_offset_val_label = tk.Label(
            phase_offset_slider_row, text="φ =  0.00 rad",
            bg="#1f1f1f", fg="#d1d5db", width=14, anchor="w"
        )
        self.phase_offset_val_label.pack(side="left")
        self.phase_offset_slider = tk.Scale(
            phase_offset_slider_row,
            from_=-4 * np.pi, to=4 * np.pi,
    ...
    def _on_phase_offset_toggle(self):
        ...
        enabled = self.var_phase_offset_enabled.get()
        self.phase_offset_slider.config(state="normal" if enabled else "disabled")
        if not enabled:
            self.var_phase_offset.set(0.0)
            self.phase_offset_val_label.config(text="φ =  0.00 rad")
        self._estimate_from_rings()
        self.fig.canvas.draw_idle()

    def _on_phase_offset_change(self, val):
        """Update the phase-offset label and re-run the Reimer analysis."""
        phi = float(val)
    ...
    def _estimate_from_rings(self):
        ...
        phi_reimer = (float(_phi_var.get())
                      if (_phi_en and _phi_en.get() and _phi_var) else 0.0)
    ...
    def _apply_reimer_view(self, q_all, n_arr, types, incl, wl):
        ...
        if view and view.get() == 'Phase':
            ...
            _phi2 = (float(_phi_var2.get())
                     if (_phi_en2 and _phi_en2.get() and _phi_var2) else 0.0)
    ...
    def _draw_phase_view(self, q_all, n_arr, types, incl, wl):
        ...
        _phi_var     = getattr(self, 'var_phase_offset', None)
        phi = float(_phi_var.get()) if (_phi_enabled and _phi_enabled.get() and _phi_var) else 0.0
